...
public sealed class SectionCrudTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public SectionCrudTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Create_Update_Delete_Section_HappyPath()
    {
        ...
        var sectionId = createDoc.RootElement.GetProperty("id").GetGuid();
        ...
        var updatePayload = JsonSerializer.Serialize(new
        {
            ...
            code = sectionCode,
            name = "Zone A Updated",
            description = "Primary area updated"
        });

        var updateResponse = await client.PutAsync($"/api/sections/{sectionId}", new StringContent(updatePayload, Encoding.UTF8, "application/json"));
        updateResponse.IsSuccessStatusCode.Should().BeTrue();

        var deleteResponse = await client.DeleteAsync($"/api/sections/{sectionId}");
        deleteResponse.IsSuccessStatusCode.Should().BeTrue();

        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var section = await db.Sections.AsNoTracking().SingleAsync(s => s.Id == sectionId);
        section.IsActive.Should().BeFalse();
        section.SectorId.Should().Be(sectorId);
    }

    [Fact]
    public async Task Delete_Unknown_Section_Should_Return_NotFound()
    {
        var client = _factory.CreateClient();
        var response = await client.DeleteAsync($"/api/sections/{Guid.NewGuid()}");
