...
public sealed class AdminUserSeedTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task AdminUser_Should_Be_Seeded()
    {
        ...
        var repo = scope.ServiceProvider.GetRequiredService<DevcraftWMS.Application.Abstractions.Auth.IUserRepository>();
        var hasher = scope.ServiceProvider.GetRequiredService<DevcraftWMS.Application.Abstractions.Auth.IPasswordHasher>();
        var logger = scope.ServiceProvider.GetRequiredService<ILogger<DevcraftWMS.Infrastructure.Auth.AdminUserSeeder>>();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var db2 = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        ReferenceEquals(db, db2).Should().BeTrue();
        var options = Options.Create(new DevcraftWMS.Infrastructure.Auth.AdminUserOptions
        {
            Email = "admin@admin.com.br",
            Password = "Naotemsenha0!",
            FullName = "System Administrator"
        });
        var seeder = new DevcraftWMS.Infrastructure.Auth.AdminUserSeeder(repo, hasher, options, logger);
