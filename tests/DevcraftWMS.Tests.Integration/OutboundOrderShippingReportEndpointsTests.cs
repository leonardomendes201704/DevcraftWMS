...
public sealed class OutboundOrderShippingReportEndpointsTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public OutboundOrderShippingReportEndpointsTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Report_Should_Return_Shipping_Summary()
    {
        ...
        var reportResponse = await client.GetAsync($"/api/outbound-orders/{orderId}/report");
        var reportBody = await reportResponse.Content.ReadAsStringAsync();
        reportResponse.IsSuccessStatusCode.Should().BeTrue(reportBody);

        using var reportDoc = JsonDocument.Parse(reportBody);
        reportDoc.RootElement.GetProperty("summary").GetProperty("totalExpected").GetDecimal().Should().Be(3m);
        reportDoc.RootElement.GetProperty("summary").GetProperty("totalShipped").GetDecimal().Should().Be(3m);
    }

    [Fact]
    public async Task ReportExport_Should_Return_Csv()
    {
        var client = _factory.CreateClient();
        ...
        var exportResponse = await client.GetAsync($"/api/outbound-orders/{orderId}/report/export");
        exportResponse.IsSuccessStatusCode.Should().BeTrue(await exportResponse.Content.ReadAsStringAsync());
    ...
    {
        var code = $"WH-REP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
    ...
    {
        var code = $"SKU-REP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
    ...
    {
        var orderNumber = $"OS-REP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
        var payload = JsonSerializer.Serialize(new
        {
            warehouseId,
            orderNumber,
            customerReference = "REF-REP-100",
            carrierName = "Carrier",
