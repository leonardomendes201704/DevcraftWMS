...
public sealed class AisleCrudTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public AisleCrudTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Create_Update_Delete_Aisle_HappyPath()
    {
        ...
        var code = $"A-{Guid.NewGuid():N}".Substring(0, 8).ToUpperInvariant();
        ...
        var createResponse = await client.PostAsync($"/api/sections/{sectionId}/aisles", new StringContent(createPayload, Encoding.UTF8, "application/json"));
        var createBody = await createResponse.Content.ReadAsStringAsync();
        createResponse.IsSuccessStatusCode.Should().BeTrue(createBody);

        using var createDoc = JsonDocument.Parse(createBody);
        var aisleId = createDoc.RootElement.GetProperty("id").GetGuid();

        var updatePayload = JsonSerializer.Serialize(new
        {
            sectionId,
            code,
            name = "Main Aisle Updated"
        });

        var updateResponse = await client.PutAsync($"/api/aisles/{aisleId}", new StringContent(updatePayload, Encoding.UTF8, "application/json"));
        updateResponse.IsSuccessStatusCode.Should().BeTrue();

        var deleteResponse = await client.DeleteAsync($"/api/aisles/{aisleId}");
        deleteResponse.IsSuccessStatusCode.Should().BeTrue();

        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var aisle = await db.Aisles.AsNoTracking().SingleAsync(a => a.Id == aisleId);
        aisle.IsActive.Should().BeFalse();
        aisle.SectionId.Should().Be(sectionId);
    }

    [Fact]
    public async Task Delete_Unknown_Aisle_Should_Return_NotFound()
    {
        var client = _factory.CreateClient();
        var response = await client.DeleteAsync($"/api/aisles/{Guid.NewGuid()}");
