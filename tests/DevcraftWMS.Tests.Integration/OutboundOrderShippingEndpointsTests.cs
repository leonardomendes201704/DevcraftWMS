...
public sealed class OutboundOrderShippingEndpointsTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public OutboundOrderShippingEndpointsTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Ship_Should_Update_Status_To_Shipped()
    {
        ...
        var shipResponse = await client.PostAsync($"/api/outbound-orders/{orderId}/ship", new StringContent(shipPayload, Encoding.UTF8, "application/json"));
        var shipBody = await shipResponse.Content.ReadAsStringAsync();
        shipResponse.IsSuccessStatusCode.Should().BeTrue(shipBody);
        ...
        refreshedDoc.RootElement.GetProperty("status").GetInt32().Should().Be(7);
    }

    private static async Task<Guid> CreateWarehouseAsync(HttpClient client)
    {
        var code = $"WH-SHIP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
    ...
    {
        var code = $"SKU-SHIP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
    ...
    {
        var orderNumber = $"OS-SHIP-{Guid.NewGuid():N}".Substring(0, 12).ToUpperInvariant();
        var payload = JsonSerializer.Serialize(new
        {
            warehouseId,
            orderNumber,
            customerReference = "REF-SHIP-100",
            carrierName = "Carrier",
