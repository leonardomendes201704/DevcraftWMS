...
public sealed class StructureCrudTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public StructureCrudTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Create_Update_Delete_Structure_HappyPath()
    {
        ...
        var updateResponse = await client.PutAsync($"/api/structures/{structureId}", new StringContent(updatePayload, Encoding.UTF8, "application/json"));
        updateResponse.IsSuccessStatusCode.Should().BeTrue();

        var deleteResponse = await client.DeleteAsync($"/api/structures/{structureId}");
        deleteResponse.IsSuccessStatusCode.Should().BeTrue();

        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var structure = await db.Structures.AsNoTracking().SingleAsync(s => s.Id == structureId);
        structure.IsActive.Should().BeFalse();
        structure.SectionId.Should().Be(sectionId);
    }

    [Fact]
    public async Task Delete_Unknown_Structure_Should_Return_NotFound()
    {
        var client = _factory.CreateClient();
        var response = await client.DeleteAsync($"/api/structures/{Guid.NewGuid()}");
