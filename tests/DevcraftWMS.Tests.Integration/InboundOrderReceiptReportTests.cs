...
public sealed class InboundOrderReceiptReportTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public InboundOrderReceiptReportTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task GetReceiptReport_Should_Return_Lines()
    {
        var client = _factory.CreateClient();
        var inboundOrderId = await CreateInboundOrderAsync(client);

        var response = await client.GetAsync($"/api/inbound-orders/{inboundOrderId}/report");
    ...
    [Fact]
    public async Task ExportReceiptReport_Should_Return_Csv()
    {
        var client = _factory.CreateClient();
        var inboundOrderId = await CreateInboundOrderAsync(client);

        var response = await client.GetAsync($"/api/inbound-orders/{inboundOrderId}/report/export");
        var body = await response.Content.ReadAsStringAsync();
        response.IsSuccessStatusCode.Should().BeTrue(body);
        response.Content.Headers.ContentType?.MediaType.Should().Be("text/csv");
        body.Should().Contain("ProductCode,ProductName,Uom,LotCode,ExpirationDate,ExpectedQty,ReceivedQty,Variance");
    }
    ...
    private static async Task<Guid> CreateInboundOrderAsync(HttpClient client)
    {
        ...
        {
            warehouseId,
            asnNumber = $"ASN-REPORT-{Guid.NewGuid():N}".Substring(0, 16),
            documentNumber = "DOC-REPORT-01",
    ...
    private static async Task<Guid> CreateProductAsync(HttpClient client, Guid baseUomId)
    {
        ...
        var ean = $"{Random.Shared.Next(100000000, 999999999)}{Random.Shared.Next(1000, 9999)}";
