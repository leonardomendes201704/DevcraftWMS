...
public sealed class SectorCrudTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public SectorCrudTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Create_Update_Delete_Sector_HappyPath()
    {
        ...
        var sectorId = createDoc.RootElement.GetProperty("id").GetGuid();
        ...
        var updatePayload = JsonSerializer.Serialize(new
        {
            ...
            description = "Inbound receiving zone updated",
            sectorType = 1
        });

        var updateResponse = await client.PutAsync($"/api/sectors/{sectorId}", new StringContent(updatePayload, Encoding.UTF8, "application/json"));
        updateResponse.IsSuccessStatusCode.Should().BeTrue();

        var deleteResponse = await client.DeleteAsync($"/api/sectors/{sectorId}");
        deleteResponse.IsSuccessStatusCode.Should().BeTrue();

        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var sector = await db.Sectors.AsNoTracking().SingleAsync(s => s.Id == sectorId);
        sector.IsActive.Should().BeFalse();
        sector.WarehouseId.Should().Be(warehouseId);
    }

    [Fact]
    public async Task Delete_Unknown_Sector_Should_Return_NotFound()
    {
        var client = _factory.CreateClient();
        var response = await client.DeleteAsync($"/api/sectors/{Guid.NewGuid()}");
