...
public sealed class InventoryMovementServiceTests
{
    ...
    private sealed class FakeLocationRepository : ILocationRepository
    {
        ...
        public Task<IReadOnlyList<Location>> ListAsync(Guid structureId, int pageNumber, int pageSize, string orderBy, string orderDir, string? code, string? barcode, bool? isActive, bool includeInactive, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<Location>>(Array.Empty<Location>());
    }
