...
public sealed class ReassignPutawayTaskTests
{
    ...
    private sealed class FakeUserRepository : IUserRepository
    {
        ...
        public Task<bool> EmailExistsAsync(string email, Guid? excludeId = null, CancellationToken cancellationToken = default)
            => Task.FromResult(email.Equals(_user.Email, StringComparison.OrdinalIgnoreCase));

        public Task<int> CountAsync(string? search, bool? isActive, bool includeInactive, CancellationToken cancellationToken = default)
            => Task.FromResult(0);
